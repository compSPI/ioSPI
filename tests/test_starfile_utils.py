import pytest
from ioSPI.starfile_utils import check_star_file, starfile_opticsparams, return_names, starfile_data
import torch
import numpy as np
...
def test_starfile_data():
    ...
    rot_params={"relion_angle_rot": rot_val[:,0],
                "relion_angle_tilt": rot_val[:, 1],
                "relion_angle_psi": rot_val[:, 2]
                }

    shift_params={"shift_x": shift_val[:,0],
                "shift_y":shift_val[:,1]
                }
    ctf_params={"defocus_u":ctf_val[:,0],
                "defocus_v":ctf_val[:,1],
                "defocus_angle":ctf_val[:,2]
                }
    datalist=[]
    datalist=starfile_data(datalist, rot_params, ctf_params, shift_params, iterations, config)
    assert len(datalist)==config.batch_size
    for num,list_var in  enumerate(datalist):

        assert isinstance(list_var[0], str)
        assert normalized_mse(list_var[1], rot_params["relion_angle_rot"][num].numpy())<0.01
        assert normalized_mse(list_var[2], rot_params["relion_angle_tilt"][num].numpy())<0.01
        assert normalized_mse(list_var[3], rot_params["relion_angle_psi"][num].numpy())<0.01
        ...
        assert normalized_mse(list_var[6], 1e4*ctf_params["defocus_u"][num].numpy())<0.01
        assert normalized_mse(list_var[7], 1e4*ctf_params["defocus_v"][num].numpy())<0.01
        assert normalized_mse(list_var[8], np.radians(ctf_params["defocus_angle"][num].numpy()))<0.01
